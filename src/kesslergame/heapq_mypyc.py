...
# is the index of a leaf with a possibly out-of-order value. Restore the heap invariant.
        ...
        while len(h) > 1:
            try:
                while True:
                    ...
                    yield value
                    s[0] = next_fn()  # raises StopIteration when exhausted
                    _heapreplace_func(h, s)  # restore heap condition
            except StopIteration:
                _heappop_func(h)  # remove empty iterator
        if h:
...
def nsmallest(
    ...
    if n == 1:
        it = iter(iterable)
        sentinel = object()
        smallest: object = min(it, default=sentinel, key=key)
        return [] if smallest is sentinel else [smallest]  # type: ignore[list-item]

    # When n>=size, it's faster to use sorted()
    try:
        size = len(iterable)  # type: ignore[arg-type]
    ...
    if key is None:
        ...
        heap_data: list[tuple[T, int]] = [(elem, i) for i, elem in zip(range(n), it)]
        if not heap_data:
            return []
        _heapify_max(heap_data)
        top = heap_data[0][0]
        order = n
        _heapreplace = _heapreplace_max
        for elem in it:
            if elem < top:
                _heapreplace(heap_data, (elem, order))
                top, _order = heap_data[0]
                order += 1
        heap_data.sort()
        return [elem for (elem, _order) in heap_data]

    # General case, slowest method
    it = iter(iterable)
    heap_data_keyed: list[tuple[Any, int, T]] = [
        (key(elem), i, elem) for i, elem in zip(range(n), it)
    ]
    if not heap_data_keyed:
        return []
    _heapify_max(heap_data_keyed)
    top = heap_data_keyed[0][0]
    order = n
    _heapreplace = _heapreplace_max
    for elem in it:
        k = key(elem)
        if k < top:
            _heapreplace(heap_data_keyed, (k, order, elem))
            top, _order, _elem = heap_data_keyed[0]
            order += 1
    heap_data_keyed.sort()
    return [elem for (_k, _order, elem) in heap_data_keyed]
...
def nlargest(
    ...
    if n == 1:
        it = iter(iterable)
        sentinel = object()
        largest: object = max(it, default=sentinel, key=key)
        return [] if largest is sentinel else [largest]  # type: ignore[list-item]
    ...
    if key is None:
        it = iter(iterable)
        heap_data: list[tuple[T, int]] = [(elem, i) for i, elem in zip(range(0, -n, -1), it)]
        if not heap_data:
            return []
        heapify(heap_data)
        top = heap_data[0][0]
        ...
        for elem in it:
            if top < elem:
                ...
                top, _order = heap_data[0]
                order -= 1
        heap_data.sort(reverse=True)
        return [elem for (elem, _order) in heap_data]

    # General case, slowest method
    it = iter(iterable)
    heap_data_keyed: list[tuple[Any, int, T]] = [
        (key(elem), i, elem) for i, elem in zip(range(0, -n, -1), it)
    ]
    if not heap_data_keyed:
        return []
    heapify(heap_data_keyed)
    top = heap_data_keyed[0][0]
    order = -n
    _heapreplace = heapreplace
    for elem in it:
        ...
        if top < k:
            ...
            top, _order, _elem = heap_data_keyed[0]
            order -= 1
    heap_data_keyed.sort(reverse=True)
    return [elem for (_k, _order, elem) in heap_data_keyed]
